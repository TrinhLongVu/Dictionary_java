 ...
 * @author Admin
 */
public class RandomDefinition extends JPanel {
    ...
    private String slangDefinition;

    RandomDefinition(HashMap<String, ArrayList<String>> d) {
        Dicts = d;
        UI();
    }
    ...
    private class ButtonClickListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (b1.getText().equals(slangDefinition) && "1".equals(command)) {
                JOptionPane.showMessageDialog(null, "Correct Answer", "Message", JOptionPane.INFORMATION_MESSAGE);
            } else if (b2.getText().equals(slangDefinition) && "2".equals(command)) {
                JOptionPane.showMessageDialog(null, "Correct Answer", "Message", JOptionPane.INFORMATION_MESSAGE);
            } else if (b3.getText().equals(slangDefinition) && "3".equals(command)) {
                JOptionPane.showMessageDialog(null, "Correct Answer", "Message", JOptionPane.INFORMATION_MESSAGE);
            } else if (b4.getText().equals(slangDefinition) && "4".equals(command)) {
                JOptionPane.showMessageDialog(null, "Correct Answer", "Message", JOptionPane.INFORMATION_MESSAGE);
            } else {
    ...
    private void UI() {
        ...
        slangDefinition = result.get(0);
        slang = new JLabel(Dicts.get(slangDefinition).get(0), SwingConstants.CENTER);
        int temp = rand.nextInt(result.size());
        b1 = new JButton(result.get(temp));
        result.remove(temp);

        temp = rand.nextInt(result.size());
        b2 = new JButton(result.get(temp));
        result.remove(temp);

        temp = rand.nextInt(result.size());
        b3 = new JButton(result.get(temp));
        result.remove(temp);

        temp = rand.nextInt(result.size());
        b4 = new JButton(result.get(temp));
        result.remove(temp);
